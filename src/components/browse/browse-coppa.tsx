...
import { BROWSE_COPPA_POINTS } from "@/data/browse";

import { ShieldIcon } from "./browse-icons";

/** Parent-supervision (COPPA) band: reassuring headline + three control points. */
export function BrowseCoppa() {
  return (
    <Section className="bg-sand py-[12vh]">
      <Reveal>
        <div className="relative grid grid-cols-[0.8fr_1.2fr] items-center gap-11 overflow-hidden rounded-[24px] border border-line bg-ivory p-[52px] max-[860px]:grid-cols-1 max-[860px]:gap-7 max-[860px]:p-[34px]">
          <div aria-hidden="true" className="absolute inset-0 z-0 opacity-[0.14]">
            <Image
              src="/assets/browse/images/coppa-bg.jpg"
              alt=""
              fill
              sizes="100vw"
